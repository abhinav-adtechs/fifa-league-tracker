...
export const MatchList: React.FC<MatchListProps> = ({ matches, players }) => {
  ...
  const filteredMatches = useMemo(() => {
    ...
    if (h2hPlayerA && h2hPlayerB) {
      ...
      m = m.filter(
        (match) =>
          match.player1Id === filterPlayer || match.player2Id === filterPlayer,
      );
    }
  ...
  const last30DaysChartData = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const count = filteredMatches.filter(
        (m) => getDayKey(m.timestamp) === key,
      ).length;
      const d = new Date(t);
      out.push({
    ...
    return (
      <div className="glass-card text-center py-16 px-6">
        ...
        <p className="text-sm text-text-muted mt-1">
          Record your first match to see results here.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      ...
      <div className="glass-card p-3 sm:p-4 flex flex-col gap-3">
        ...
        <p className="text-xs text-text-secondary -mt-1">
          Select two players to compare stats and view matches between them.
        </p>
      ...
      {h2h && h2hPlayerA && h2hPlayerB && (
        <div className="glass-card gradient-border p-4 sm:p-6 animate-fade-up">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-[11px] font-semibold text-accent-green uppercase tracking-widest">
              Head to Head
            </span>
          </div>
          <div className="grid grid-cols-[1fr_auto_1fr] gap-4 sm:gap-6 items-start">
            ...
            <div className="flex flex-col items-center text-center">
              {(() => {
                const pa = getPlayer(h2h.playerAId);
                return (
                  <>
                    <img
                      src={pa?.avatarUrl}
                      ...
                      onError={(e) => {
                        ...
                        t.src = `https://api.dicebear.com/7.x/avataaars/svg?seed=${pa?.name}`;
                      }}
                    />
                    <span className="font-bold text-text-primary text-sm sm:text-base">
                      {pa ? playerWithTeamLabel(pa) : ""}
                    </span>
                  </>
                );
              })()}
              <div className="mt-3 flex items-center gap-2">
                <span className="text-lg sm:text-xl font-extrabold text-accent-green">
                  {h2h.winsA}
                </span>
                ...
                <span className="text-text-muted">·</span>
                <span className="text-lg sm:text-xl font-extrabold text-accent-red">
                  {h2h.winsB}
                </span>
              ...
              <div className="mt-2 text-xs font-mono text-text-muted">
                ...
                {h2h.gdA !== 0 && (
                  <span
                    className={
                      h2h.gdA > 0
                        ? " text-accent-green ml-1"
                        : " text-accent-red ml-1"
                    }
                  >
                    (GD {h2h.gdA > 0 ? "+" : ""}
                    {h2h.gdA})
            ...
            <div className="flex flex-col items-center text-center">
              {(() => {
                const pb = getPlayer(h2h.playerBId);
                return (
                  <>
                    <img
                      src={pb?.avatarUrl}
                      ...
                      onError={(e) => {
                        const t = e.target as HTMLImageElement;
                        t.src = `https://api.dicebear.com/7.x/avataaars/svg?seed=${pb?.name}`;
                      }}
                    />
                    <span className="font-bold text-text-primary text-sm sm:text-base">
                      {pb ? playerWithTeamLabel(pb) : ""}
                    </span>
                  </>
                );
              })()}
            ...
            <div key={dateKey} className="date-section-card">
              ...
              <div>
                {dayMatches.map((match) => {
                  ...
                  const p1Team = getDisplayTeamName(p1?.teamName);
                  const p2Team = getDisplayTeamName(p2?.teamName);
                  ...
                  return (
                    <div key={match.id} className="match-row-compact">
                      <div
                        className={`flex-1 min-w-0 flex items-center gap-2 justify-end ${p1Won ? "opacity-100" : isDraw ? "opacity-80" : "opacity-50"}`}
                      >
                        <div className="min-w-0 flex flex-col items-end text-right">
                          <span className="text-[10px] sm:text-[11px] font-medium text-text-primary truncate">
                            {p1?.name || "Unknown"}
                          </span>
                          {p1Team && (
                      ...
                      <div className="flex items-center gap-1.5 shrink-0 px-2">
                        <span
                          className={`text-sm font-bold font-mono tabular-nums w-5 text-right ${p1Won ? "text-accent-green" : "text-text-primary"}`}
                        >
                          {match.score1}
                        </span>
                        <span className="text-[10px] text-text-muted">–</span>
                        <span
                          className={`text-sm font-bold font-mono tabular-nums w-5 text-left ${p2Won ? "text-accent-green" : "text-text-primary"}`}
                        >
                          {match.score2}
                        </span>
                      ...
                      <div
                        className={`flex-1 min-w-0 flex items-center gap-2 justify-start ${p2Won ? "opacity-100" : isDraw ? "opacity-80" : "opacity-50"}`}
                      >
                        <img
                          src={p2?.avatarUrl}
                          alt=""
