...
export const Dashboard: React.FC<DashboardProps> = ({ players, matches }) => {
  ...
  const trajectoryData = useMemo(() => {
    ...
    sortedMatches.forEach((m, idx) => {
      ...
      const row: { match: number | string; [k: string]: number | string } = {
        match: idx + 1,
      };
      players.forEach((p) => {
        ...
        if (trajectoryView === "TABLE") {
          ...
          row[p.name] =
            playedVal > 0 ? Math.round((pointVal / playedVal) * 100) / 100 : 0;
        } else {
          row[p.name] =
            Math.round(
              getNormalisedScoreFromStats(playedVal, pointVal, gdVal) * 100,
            ) / 100;
        }
      });
      history.push(row);
  ...
  const leagueStatsCards = [
    ...
    },
    {
      ...
      value: leagueStats.totalMatches
        ? leagueStats.avgGoalsPerMatch.toFixed(1)
        : "0",
      icon: BarChart3,
  ...
  const playerStatsCards = [
    ...
    },
    {
      ...
      subtitle:
        bestDefender && bestDefender.played > 0
          ? `${(bestDefender.ga / bestDefender.played).toFixed(1)} GA/G`
          : undefined,
      icon: Target,
      color: "text-accent-blue",
      bgColor: "bg-accent-blue/10",
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {leagueStatsCards.map((stat, i) => (
            <div key={i} className="stat-card group">
              ...
              <div
                className={`text-xl sm:text-2xl font-extrabold ${stat.color} font-mono tracking-tight truncate`}
              >
                {stat.value}
              </div>
              {stat.subtitle && (
        ...
        <div className="flex flex-col gap-3 mb-5">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-[10px] font-mono text-text-muted ml-auto">
              {trajectoryView === "TABLE"
                ? "Points over time"
                : trajectoryView === "PPG"
                  ? "PPG over time"
                  : "Normalised score over time"}
            </span>
          </div>
