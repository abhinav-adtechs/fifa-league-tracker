...
import {
  ...
  getCalendarHistogramData,
} from "../utils/myPerformance";
import {
  BarChart,
  Bar,
  XAxis,
...
export const MyPerformance: React.FC<MyPerformanceProps> = ({
    ...
    if (currentAdmin?.name) {
      const match = players.find(
        (p) =>
          ...
          currentAdmin.name.toLowerCase().trim(),
      );
  ...
  useEffect(() => {
    if (defaultPlayerId && !selectedPlayerId)
  ...
  const calendarData = useMemo(
    () =>
      selectedPlayerId
        ? getCalendarHistogramData(selectedPlayerId, matches, 30)
        : [],
    [selectedPlayerId, matches],
  );
    ...
    return (
      <div className="glass-card text-center py-16 px-6">
        ...
        <p className="text-sm text-text-muted mt-1">
          Add players in Squad to view performance.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-3 sm:p-4">
        ...
        <p className="text-xs text-text-secondary mb-3">
          Select your player to see your calendar, consistency, goals, and
          biggest results.
        </p>
        <select
              ...
              <div>
                ...
                {stats.biggestWinOpponentId && (
                  <div className="text-[11px] text-text-muted mt-0.5">
                    vs{" "}
                    {players.find((p) => p.id === stats.biggestWinOpponentId)
                      ?.name ?? "Unknown"}
                  </div>
                )}
              </div>
            ...
            <div className="glass-card p-4 flex items-center gap-4">
              ...
              <div>
                ...
                {stats.biggestLossOpponentId && (
                  <div className="text-[11px] text-text-muted mt-0.5">
                    vs{" "}
                    {players.find((p) => p.id === stats.biggestLossOpponentId)
                      ?.name ?? "Unknown"}
                  </div>
                )}
              </div>
